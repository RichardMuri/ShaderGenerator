...
def build_dataset_vocab(examples, transition_system, src_size=5000, code_size=5000, primitive_size=5000, src_cutoff=0, code_cutoff=0, primitive_cutoff=0):
    src_vocab = VocabEntry.from_corpus(
        [e.src_toks for e in examples], size=src_size, freq_cutoff=src_cutoff)
    ...
    code_vocab = VocabEntry.from_corpus(
        code_tokens, size=code_size, freq_cutoff=0)

    type_primitive_tokens = []
    for tree in map(lambda x: x.tgt_actions, examples):
        type_primitive_tokens.extend(
            extract_primitive_tokens(tree, transition_system))

    primitive_entries = {}
    for prim_type in transition_system.grammar.primitive_types:
        type_entry = PrimitiveVocabEntry.from_corpus(
            [[v for t, v in type_primitive_tokens if t == prim_type]], size=primitive_size, freq_cutoff=primitive_cutoff)
...
def extract_primitive_tokens(root, transition_system):
    ...
    def _exract_primitive_tokens_helper(node):
        if isinstance(node, list):
            for item in node:
                if transition_system.grammar.is_primitive_type(item.action.type):
                    results.append((item.action.type, item.action.choice))
                else:
